...
/*   Updated: 2022/08/14 09:11:13 by ccambium         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	init_mutex2(t_philosopher *philo)
{
	...
	philo->fork_mutex = (pthread_mutex_t *)
		ft_malloc(sizeof(pthread_mutex_t), philo);
	if (!philo->fork_mutex || pthread_mutex_init(philo->fork_mutex, NULL))
	{
		ft_error("Could not create fork_mutex");
		abort_philo(philo, 0);
		return (0);
	}
	return (1);
